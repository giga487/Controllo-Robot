...
syms L1 L2 L3 L5 L4 Lhead x1 x2 x3 x4 x5 x_head ...
    l_head kr Ixx_b Iyy_b Izz_b...
    m_b1 m_b2 m_b3 m_b4 m_b5 m_b6 dx1 dx2 dx3 dx4 dx5 dx_head g0 real
...
TG1 = matrixDH(L1/2,0,0,x1+angle0_1);
pG1 = TG1(1:3,4);
rG1 = TG1(1:3,1:3);
...
TG2 = TG1*matrixDH(L2/2,0,0,x2+angle0_2);
pG2 = TG2(1:3,4);
rG2 = TG2(1:3,1:3);
...
TG3 = TG2*matrixDH(L3/2,0,0,x3+angle0_3);
pG3 = TG3(1:3,4);
rG3 = TG3(1:3,1:3);
...
TG4 = TG3*matrixDH(L4/2,0,0,x4+angle0_4);
...
TG5 = TG4*matrixDH(L5/2,0,0,x5+angle0_5);
...
TGHead = TG2*matrixDH(Lhead/2,0,0,x_head+angle0_head);
...
BJoint = (m_b1*(JpG1')*JpG1 + (JoG1')*rG1*Inertia_f(m_b1,L1)*(rG1')*JoG1+...
        m_b2*(JpG2')*JpG2 + (JoG2')*rG2*Inertia_f(m_b2,L2)*(rG2')*JoG2+...
        m_b3*(JpG3')*JpG3 + (JoG3')*rG3*Inertia_f(m_b3,L3)*(rG3')*JoG3+...
        m_b4*(JpG4')*JpG4 + (JoG4')*rG4*Inertia_f(m_b4,L4)*(rG4')*JoG4+...
        m_b5*(JpG5')*JpG5 + (JoG5')*rG5*Inertia_f(m_b5,L5)*(rG5')*JoG5+...
        m_b6*(JpGHead')*JpGHead + (JoGHead')*rGHead*Inertia_f(m_b6,Lhead)*(rGHead')*JoGHead);
